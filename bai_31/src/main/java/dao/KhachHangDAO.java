...
public class KhachHangDAO implements DAO<KhachHang>{
	...
	public boolean update(KhachHang object) {
		...
		Connection c = JDBCUtil.connect();
		try {
			String sql = "UPDATE khachhang SET tendangnhap=?,matkhau=?,hoten=?,diachi=?,diachimuahang=?,diachinhanhang=?,ngaysinh=?,sodienthoai=?,gioitinh=?,nhanbantin=? WHERE makhachhang=?";
			PreparedStatement pst = c.prepareStatement(sql);
