...
public class KhachHangDAO implements DAO<KhachHang>{
	...
	public KhachHang selectById(String object) {
		...
		try {
			...
			while(rs.next()) {
				...
				target = new KhachHang(maKhachHang, tenDangNhap, matKhau, hoTen, diaChi, diaChiMuaHang, diaChiNhanHang, ngaySinh, soDienThoai,email, gioiTinh, nhanBanTin,maXacThuc,thoiGianHieuLuc,trangThaiXacThuc,hinhAnh);
			}
			JDBCUtil.disconnect(c);
		}
		catch (Exception e) {
	...
	public boolean insert(KhachHang object) {
		...
		try {
			...
			pst.setString(16,object.getHinhAnh());
	...
	public boolean changeImage(KhachHang kh) {
		KhachHang target = selectById(kh.getMaKhachHang());
		if(target == null)
			return false;
		...
		Connection c = JDBCUtil.connect();
		try {
			String sql = "UPDATE khachhang SET hinhanh=? WHERE makhachhang=?";
			PreparedStatement pst = c.prepareStatement(sql);
			
			
			pst.setString(1, kh.getHinhAnh());
			pst.setString(2, kh.getMaKhachHang());
